...
# ---------------------------------------------------------------------------------
# Absolute paths to your Excel and logo
DATA_PATH = Path(r"C:\Users\HP\Downloads\PythonStreamlit-main\PythonStreamlit-main\Adidas.xlsx")
LOGO_PATH = Path(r"C:\Users\HP\Downloads\PythonStreamlit-main\PythonStreamlit-main\adidas-logo.jpg")
...
with st.sidebar:
    ...
    # Retailer filter
    retailer_list = sorted(df["Retailer"].dropna().unique()) if "Retailer" in df.columns else []
    selected_retailer = st.multiselect("Retailer", retailer_list, default=retailer_list)

    # State filter
    state_list = sorted(df["State"].dropna().unique()) if "State" in df.columns else []
...
if date_range and "InvoiceDate" in df_filtered.columns:
...
with col3:
    ...
    st.markdown('<div class="section-subtitle">Compare which partners drive the most revenue.</div>', unsafe_allow_html=True)
    ...
    if not data_retailer.empty:
        ...
        st.plotly_chart(fig_retailer, width="stretch")
...
with col4:
    ...
    st.markdown('<div class="section-subtitle">Track how revenue evolves across months.</div>', unsafe_allow_html=True)
    ...
    if "Month_Year" in df_filtered.columns:
        ...
        st.info("InvoiceDate / Month_Year not available in the dataset.")

st.markdown("---")

# ---------------------------------------------------------------------------------
# ROW 2 – STATE + UNITS (COMBO CHART) & PRODUCT / METHOD ANALYSIS
# ---------------------------------------------------------------------------------
col5, col6 = st.columns([0.55, 0.45])

with col5:
    st.markdown('<div class="section-title">State Performance</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-subtitle">View sales and units sold by state in one glance.</div>', unsafe_allow_html=True)
    ...
    if {"State", "TotalSales", "UnitsSold"}.issubset(df_filtered.columns):
        ...
        st.warning("State / UnitsSold columns not available.")

with col6:
    st.markdown('<div class="section-title">Product & Sales Mix</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-subtitle">Which products or channels dominate revenue?</div>', unsafe_allow_html=True)

    # Product bar (if available)
...
# ---------------------------------------------------------------------------------
# ROW 3 – TREEMAP (REGION / CITY) & RAW DATA
# ---------------------------------------------------------------------------------
col7, col8 = st.columns([0.6, 0.4])

with col7:
    st.markdown('<div class="section-title">Region & City Heatmap</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-subtitle">Drill into the Adidas footprint across regions and cities.</div>', unsafe_allow_html=True)
    ...
    if {"Region", "City", "TotalSales"}.issubset(df_filtered.columns):
        ...
        st.warning("Region / City / TotalSales not available for Treemap.")

with col8:
    st.markdown('<div class="section-title">Raw Dataset (Filtered)</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-subtitle">Inspect individual transactions behind the charts.</div>', unsafe_allow_html=True)
